...
const fakeAdapter = {
  ...
  async getObjectAsync(id) { const k = `${this.namespace}.${id}`; return store.has(`obj:${k}`) ? store.get(`obj:${k}`) : null; },
  async setObjectNotExistsAsync(id, obj) {
    const k = `${this.namespace}.${id}`;
    if (!store.has(`obj:${k}`)) store.set(`obj:${k}`, obj);
  },
  ...
  async getStateAsync(id) { const k = `${this.namespace}.${id}`; return store.has(`state:${k}`) ? store.get(`state:${k}`) : null; },
  async getStatesAsync(pattern) {
    ...
    for (const [k, v] of store.entries()) {
      ...
      if (regex.test(rel)) out[id] = v;
    }
    return out;
  },
  async getForeignStateAsync(id) { return store.has(`state:${id}`) ? store.get(`state:${id}`) : null; },
  async setStateAsync(id, val) { const k = `${this.namespace}.${id}`; store.set(`state:${k}`, val); return val; },
  async setForeignStateAsync(id, val) { store.set(`state:${id}`, val); return val; },
  async subscribeForeignStatesAsync(_pattern) { /* no-op im Test - kein echter Subscription-Mechanismus nötig */ },
  async subscribeStatesAsync(_pattern) { /* no-op im Test */ },
  async sendToAsync(instance, payload) { sentMessages.push({ instance, payload }); },
  sendTo(instance, command, message, callback) {
    ...
    if (instance === this.config.javascriptInstance && command === 'toScript') {
      if (message.script === 'script.js.Test.respond' || message.script === 'script.js.Telegram.mod_shortname_test') {
...
async function run() {
  ...
  console.log('Module geladen:', fakeAdapter.modules.map((m) => m.id));
  ...
  await fakeAdapter.setObjectNotExistsAsync('users.123456.approved', { type: 'state', common: { name: 'approved', type: 'boolean', role: 'indicator', read: true, write: true }, native: {} });
  ...
  if (permAfter?.val !== true) throw new Error('Rechte-Toggle hat nicht geschrieben!');
  ...
  await setMenu(fakeAdapter, 'main', { title: '🏠 Test', groupableExcludeTypes: ['warn', 'error', 'priceChange', 'irgendwas'], rows: [[{ text: '⬅️ Zurück', cmd: 'TG:NAV:BACK' }]] });
  ...
  if (sentMessages.length !== 0) throw new Error('notify() hat trotz abgeschalteter Präferenz gesendet!');
  ...
  if (sentMessages.length !== 1) throw new Error('notify() hat trotz eingeschalteter Präferenz NICHT gesendet!');
  ...
  if (areasBefore.spontanBereich) throw new Error('Bereich existierte schon vor dem ersten Aufruf - Test unbrauchbar');
  ...
  console.log('--- getValidAreas() nach Auto-Entdeckung (sollte spontanBereich NICHT enthalten) ---', Object.keys(areasAfterDiscovery));
  if (areasAfterDiscovery.spontanBereich) throw new Error('Unfreigeschalteter Bereich taucht fälschlich in getValidAreas() auf!');

  const pending = fakeAdapter.notify.getPendingAreas();
  console.log('--- getPendingAreas() ---', Object.keys(pending));
  if (!pending.spontanBereich) throw new Error('Bereich fehlt in getPendingAreas()!');
  ...
  if (!persisted.spontanBereich) throw new Error('Bereich wurde nicht in notify.areas persistiert!');
  ...
  if (sentMessages.length !== 0) throw new Error('Unfreigeschalteter Bereich hat trotzdem zugestellt!');
  ...
  if (sentMessages.length !== 1) throw new Error('Freigeschalteter Bereich liefert nicht zu!');
  ...
  if (sentMessages.length !== 0) throw new Error('Ausgeschaltete Präferenz wurde ignoriert (sollte blockieren)!');
  ...
  if (sentMessages.length !== 1) throw new Error('Override hat die ausgeschaltete Präferenz nicht überschrieben!');
  ...
  if (!notifyTexts2.some((t) => t.includes('Preisänderung'))) throw new Error('Anzeigename wurde nicht verwendet!');
  if (notifyTexts2.some((t) => t.includes('priceChange'))) throw new Error('Roher Typ-Name taucht trotz Label noch auf!');
  ...
  if (!notifyTexts3.some((t) => t.includes('Testbereich'))) throw new Error('Bereichsname wurde nicht verwendet!');
  ...
  if (full.spontanBereich.areaLabel !== 'Testbereich') throw new Error('areaLabel fehlt in getAllAreasFull()!');
  if (!full.spontanBereich.override.includes('irgendwas')) throw new Error('override fehlt in getAllAreasFull()!');
  ...
  if (textsWithOverride.some((t) => t.includes('irgendwas'))) throw new Error('Override-Typ taucht trotzdem im Menü auf!');
  ...
  if (!deleted) throw new Error('deleteArea() meldet Fehlschlag!');
  const fullAfterDelete = fakeAdapter.notify.getAllAreasFull();
  if (fullAfterDelete.spontanBereich) throw new Error('Bereich existiert nach deleteArea() immer noch!');
  ...
  if (!percentTexts.includes('0%') || !percentTexts.includes('100%')) throw new Error('Prozent-Range hat nicht 0-100 erzeugt!');
  if (!percentTexts.includes('✅ 40%')) throw new Error('Aktueller Wert (40%) wurde nicht mit ✅ markiert!');
  if (percentTexts.filter((t) => t.startsWith('✅')).length !== 1) throw new Error('Mehr als ein Wert wurde als aktuell markiert!');
  if (percentTexts.some((t) => t.includes('Zurück') || t.includes('Hauptmenü'))) {
    throw new Error('Prozent-Stufen-Menü hat noch Zurück/Hauptmenü-Buttons - die wurden bewusst entfernt (Bestätigung springt direkt zurück)!');
  }
  // rowLength: 3 -> erste Zeile hat 3 Buttons (0,20,40)
  if (percentRows[0].length !== 3) throw new Error('rowLength wurde bei Prozent-Stufen nicht beachtet!');
  ...
  const numberRows = await buildAutoRows(
    fakeAdapter,
    { source: 'numberRange', datapoint: '0_userdata.0.TestSoll', min: 16, max: 28, step: 4, unit: '°C', rowLength: 4 },
    '123456',
  );
  const numberTexts = numberRows.flat().map((b) => b.text);
  console.log('--- Zahlenbereich 16-28°C Schritt 4 (aktuell 24°C) ---', numberTexts);
  if (!numberTexts.includes('16°C') || !numberTexts.includes('28°C')) throw new Error('Zahlenbereich hat nicht die erwarteten Grenzwerte erzeugt!');
  if (!numberTexts.includes('✅ 24°C')) throw new Error('Aktueller Wert (24°C) wurde nicht markiert!');
  ...
  if (numberTextsDesc[0] !== '28°C') throw new Error('Absteigender Zahlenbereich beginnt nicht beim höheren Wert!');
  ...
  if (rolloState?.val !== 60) throw new Error('TG:VALSET: hat den Wert nicht auf den Datenpunkt geschrieben!');
  if (rolloState?.ack !== false) throw new Error('TG:VALSET: hätte mit ack:false schreiben müssen!');
  const afterSetMsg = sentMessages.at(-1);
  console.log('--- Nach TG:VALSET: Text/Tastatur ---', afterSetMsg?.payload?.text, '| inline_keyboard? ---', !!afterSetMsg?.payload?.reply_markup?.inline_keyboard);
  if (afterSetMsg?.payload?.text !== '✅ 60% gesetzt.') throw new Error('TG:VALSET: Standard-Bestätigungstext ("✅ 60% gesetzt.") stimmt nicht!');
  if (afterSetMsg?.payload?.reply_markup?.inline_keyboard) throw new Error('TG:VALSET: Bestätigung landet fälschlich wieder im Inline-Menü statt im Elternmenü (main)!');
  const currentMenuAfterSet = await fakeAdapter.getStateAsync('runtime.currentMenu');
  if (currentMenuAfterSet?.val !== 'main') throw new Error('TG:VALSET: currentMenu wurde nicht auf das Elternmenü ("main") gesetzt!');
  ...
  if (plainOpenMsg?.payload?.text?.includes('{value}') || plainOpenMsg?.payload?.text?.includes('{unit}')) {
    throw new Error('Normales Öffnen des Menüs zeigt unaufgelöste {value}/{unit}-Platzhalter aus confirmMessage - Bug ist zurück!');
  }
  ...
  if (rolloStateE2E?.val !== 80) throw new Error('TG:VALSET über handleIncoming() (echter Inline-Button-Tap) hat NICHT funktioniert!');
  const e2eMsg = sentMessages.at(-1);
  if (e2eMsg?.payload?.text?.includes('Unbekannter Befehl')) {
    throw new Error('handleIncoming() hat den Inline-Button-Tap als "Unbekannter Befehl" abgelehnt statt an dispatchCommand weiterzuleiten!');
  }
  ...
  if (numpadModeState?.val !== 'datapoint_numpad') throw new Error('TG:VALCUSTOM hat nicht den Ziffernblock-Eingabemodus gestartet!');
  const numpadCtx = JSON.parse((await fakeAdapter.getStateAsync('runtime.inputContext'))?.val || '{}');
  if (numpadCtx.datapoint !== '0_userdata.0.TestRollo') throw new Error('TG:VALCUSTOM hat den falschen Datenpunkt im Eingabekontext hinterlegt!');
  if (numpadCtx.menuKey !== 'main') throw new Error('TG:VALCUSTOM hat nicht das Elternmenü ("main") als Rücksprungziel hinterlegt!');
  const numpadMsg = sentMessages.at(-1);
  console.log('--- TG:VALCUSTOM: Ziffernblock-Prompt ---', numpadMsg?.payload?.text);
  if (!numpadMsg?.payload?.reply_markup?.keyboard) throw new Error('TG:VALCUSTOM hat keinen Ziffernblock (normale Tastatur) geschickt!');
  ...
  if (rolloStateAfterNumpad?.val !== 35) throw new Error('TG:VALCUSTOM hat den per Ziffernblock eingegebenen Wert nicht korrekt geschrieben!');
  const numpadConfirmMsg = sentMessages.at(-1);
  console.log('--- TG:VALCUSTOM: Bestätigung nach Eingabe ---', numpadConfirmMsg?.payload?.text);
  if (numpadConfirmMsg?.payload?.text !== '✅ 35% gesetzt.') throw new Error('TG:VALCUSTOM hat den Standard-Bestätigungstext ({value}/{unit}) nicht korrekt aufgelöst!');
  ...
  if (adminRolloState) throw new Error('TG:VALSET hat die Menü-Berechtigung (perm) nicht durchgesetzt - unprivilegierter Nutzer konnte schreiben!');
  ...
  if (afterDelete.includes('test_menu')) throw new Error('test_menu wurde nicht aus dem Index entfernt!');
  if (!afterDelete.includes('main')) throw new Error('main wurde fälschlich mit entfernt!');
  const testMenuState = await fakeAdapter.getStateAsync('registry.test_menu');
  if (testMenuState) throw new Error('registry.test_menu-State existiert nach Löschen noch!');
  ...
  console.log('--- Skript-Callback mit html:true: parse_mode ---', htmlCallbackMsg?.payload?.parse_mode, '|', htmlCallbackMsg?.payload?.text);
  if (htmlCallbackMsg?.payload?.parse_mode !== 'HTML') throw new Error('Skript-Callback mit html:true hat parse_mode nicht gesetzt!');
  if (!htmlCallbackMsg?.payload?.text?.includes('<b>Fett</b>')) throw new Error('HTML-Text aus dem Skript kam nicht unverändert an!');
  ...
  console.log('--- Skript-Callback mit noPreview:true: disable_web_page_preview ---', noPreviewCallbackMsg?.payload?.disable_web_page_preview);
  if (noPreviewCallbackMsg?.payload?.disable_web_page_preview !== true) throw new Error('Skript-Callback mit noPreview:true hat disable_web_page_preview nicht gesetzt!');
  ...
  console.log('--- Skript-Bridge-Timeout-Test abgeschlossen nach', Date.now() - timeoutStart, 'ms (Fallback griff) ---');
  ...
  console.log('--- Kurzname-Auflösung (script.js.Telegram.mod_shortname_test, javascript.5) ---', sentMessages.at(-1)?.payload?.text);
  ...
  if (dpState?.val !== true) throw new Error('Toggle hat nicht auf true geschaltet!');
  ...
  if (dpState?.val !== false) throw new Error('Toggle hat nicht zurück auf false geschaltet!');

  // Zahleneingabe mit Datenpunkt-Bezug: Button zeigt echten Ziffernblock
  await setMenu(fakeAdapter, 'main', {
    title: '🏠 Hauptmenü',
    rows: [[{ text: '🌡️ Zieltemperatur', cmd: '', datapoint: '0_userdata.0.Zieltemp', inputType: 'number', min: '10', max: '30' }]],
  });
  sentMessages.length = 0;
  await fakeAdapter.router.handleIncoming('[123456] 🌡️ Zieltemperatur');
  console.log('--- Ziffernblock nach Tastendruck ---', sentMessages.at(-1)?.payload?.text, JSON.stringify(sentMessages.at(-1)?.payload?.reply_markup?.keyboard));
  const inputModeState = await fakeAdapter.getStateAsync('runtime.inputMode');
  if (inputModeState?.val !== 'datapoint_numpad') throw new Error('inputMode wurde nicht auf datapoint_numpad gesetzt!');
  ...
  if (sentMessages.length !== countBefore) throw new Error('Bot hat auf einzelne Ziffern geantwortet - genau der Spam, der vermieden werden sollte!');
  ...
  if (dpState2) throw new Error('Ungültiger Wert wurde trotzdem geschrieben!');
  const ctxAfterReset = JSON.parse((await fakeAdapter.getStateAsync('runtime.inputContext'))?.val || '{}');
  if (ctxAfterReset.buffer !== '') throw new Error('Puffer wurde bei ungültiger Eingabe nicht komplett zurückgesetzt!');
  ...
  if (dpState2?.val !== 20) throw new Error('Ziffernblock-Wert wurde nicht korrekt geschrieben!');
  ...
  if (inputModeAfterCancel?.val) throw new Error('inputMode wurde nach Abbrechen nicht geleert!');

  // Eigene Frage statt Standardtext
  await setMenu(fakeAdapter, 'main', {
    title: '🏠 Hauptmenü',
    rows: [[{ text: '🌡️ Zieltemperatur2', cmd: '', datapoint: '0_userdata.0.Zieltemp2', inputType: 'number', prompt: 'Wie warm soll es werden?' }]],
  });
  sentMessages.length = 0;
  ...
  if (!tempReply?.includes('TG:TEMP:SHOW')) throw new Error('Button mit Platzhalter-Titel wurde beim Antippen nicht erkannt!');
  ...
  await setMenu(fakeAdapter, 'main', {
    title: '🏠 Hauptmenü',
    rows: [[{
      text: '❄️ Schnellkühlen', cmd: '', datapoint: '0_userdata.0.QuickCool', onValue: 'true', offValue: 'false',
  ...
  if (quickCoolState?.val !== true) throw new Error('Datenpunkt wurde trotz eigener Nachricht nicht geschaltet!');

  // Multi-Status-Schalter: Status bestimmt Anzeige UND Ziel-Datenpunkt (Henrik's Klima-Beispiel)
  await setMenu(fakeAdapter, 'main', {
    title: '🏠 Hauptmenü',
    rows: [[{
      text: '❄️ Schnellkühlen', cmd: '',
      icon: {
        datapoint: 'leapmotor.0.status.ac_cooling_heating',
        rules: [
          { value: 'off', emoji: '❄️', label: 'Schnellkühlen', writeDatapoint: 'leapmotor.0.cmd.quick_cool', writeValue: 'true' },
          { value: 'cooling', emoji: '🌀', label: 'AC aus', writeDatapoint: 'leapmotor.0.cmd.ac_off', writeValue: 'true' },
        ],
        fallback: '❄️',
      },
    }]],
  ...
  if (quickCool2?.val !== true) throw new Error('Multi-Status-Schalter hat im Status "off" nicht quick_cool geschrieben!');
  ...
  if (acOff?.val !== true) throw new Error('Multi-Status-Schalter hat im Status "cooling" nicht ac_off geschrieben!');

  // {status}-Platzhalter in der Nachricht wird pro Regel durch statusText ersetzt
  await setMenu(fakeAdapter, 'main', {
    title: '🏠 Hauptmenü',
    rows: [[{
      text: '❄️ Schnellkühlen', cmd: '', message: 'Schnellkühlen {status}',
      icon: {
        datapoint: 'leapmotor.0.status.ac_on2',
        rules: [
          { value: 'false', emoji: '❄️', label: 'Schnellkühlen', writeDatapoint: 'leapmotor.0.cmd.quick_cool2', writeValue: 'true', statusText: 'aktiv' },
          { value: 'true', emoji: '🌀', label: 'AC aus', writeDatapoint: 'leapmotor.0.cmd.ac_off2', writeValue: 'true', statusText: 'nicht aktiv' },
        ],
        fallback: '❄️',
      },
    }]],
  ...
  if (waescheCount !== 1) throw new Error(`Erwartete genau 1 zusammengefasste "Wäsche: Info"-Zeile, gefunden: ${waescheCount}`);
  ...
  await setMenu(fakeAdapter, 'main', {
    title: '🏠 Hauptmenü',
    rows: [[{
      text: '💡 Licht', cmd: '', datapoint: '0_userdata.0.LichtCombo', onValue: 'true', offValue: 'false',
      icon: { datapoint: '0_userdata.0.LichtStatus', rules: [{ value: 'true', emoji: '💡' }, { value: 'false', emoji: '🌑' }], fallback: '💡' },
    }]],
  });
  ...
  sentMessages.length = 0;
  await fakeAdapter.router.handleIncoming('[123456] 🌑 Licht');
  console.log('--- Status-abhängig (nur Emoji) + einfacher Datenpunkt-Fallback ---', sentMessages.at(-1)?.payload?.text);
  const comboState = await fakeAdapter.getForeignStateAsync('0_userdata.0.LichtCombo');
  if (comboState?.val !== true) throw new Error('Fallback auf einfachen Datenpunkt hat bei reinem Anzeige-Status-abhängig nicht funktioniert!');
  ...
  if (!carTitleLow?.startsWith('⚡️')) throw new Error('Menü-Icon-Operator-Regel ("<25") greift nicht - zeigt weiter Fallback statt ⚡️!');
  ...
  if (!carTitleHigh?.startsWith('🚗')) throw new Error('Menü-Icon-Fallback greift nicht mehr, wenn keine Regel passt!');

  // Menü-Titel selbst mit Platzhalter (ohne Antworttext-Feld) - genau der gemeldete Fall
  await fakeAdapter.setForeignStateAsync('0_userdata.0.Pflanzen.Strelitzie.nextWaterDueText', { val: 'in 3 Tagen', ack: true });
  ...
  if (modeState?.val !== 'auto') throw new Error('Festwert wurde nicht korrekt geschrieben!');
  ...
  if (modeState?.val !== 'auto') throw new Error('Festwert hat sich beim zweiten Antippen fälschlich geändert (sollte nicht toggeln)!');
  ...
  if (modeState?.val !== 'manual') throw new Error('Zweiter Festwert-Button hat nicht korrekt überschrieben!');
  ...
  await setMenu(fakeAdapter, 'main', {
    ...
    rows: [[{
      ...
      cmd: 'TG:PLANT:SHOW',
      icon: { datapoint: '0_userdata.0.Pflanzen.Strelitzie.wetterStatus', rules: [{ value: 'ok', emoji: '💧' }], fallback: '💧' },
    }]],
  });
  sentMessages.length = 0;
  await fakeAdapter.router.renderMenu('123456', 'main');
  ...
  fakeAdapter.sendTo = function (instance, command, message, callback) {
    if (instance === this.config.javascriptInstance && command === 'toScript' && message.script === 'script.js.Telegram.scheduleAdd') {
  ...
  console.log('--- Skript-Dialog Schritt 1 (Frage) ---', sentMessages.at(-1)?.payload?.text, JSON.stringify(sentMessages.at(-1)?.payload?.reply_markup));
  if (sentMessages.at(-1)?.payload?.text !== 'Bitte Uhrzeit im Format HH:MM eingeben:') throw new Error('Skript-Frage kam nicht an!');
  if (!sentMessages.at(-1)?.payload?.reply_markup?.remove_keyboard) throw new Error('Tastatur wurde bei Skript-Rückfrage nicht entfernt!');
  ...
  if (!sentMessages.at(-1)?.payload?.text?.includes('Ungültiges Format')) throw new Error('Verkettung bei ungültiger Eingabe hat nicht funktioniert!');
  ...
  if (!sentMessages.at(-1)?.payload?.text?.includes('10:30')) throw new Error('Finale Bestätigung kam nicht korrekt an!');
  const inputModeAfterDialog = await fakeAdapter.getStateAsync('runtime.inputMode');
  if (inputModeAfterDialog?.val) throw new Error('inputMode wurde nach Dialog-Ende nicht geleert!');
  ...
  fakeAdapter.sendTo = function (instance, command, message, callback) {
    if (instance === this.config.javascriptInstance && command === 'toScript' && message.script === 'script.js.Telegram.calib') {
      if (!message.data.isReply) {
  ...
  await setMenu(fakeAdapter, 'balcony_calib', { title: '🧪 Kalibrieren', parent: 'main', rows: [[{ text: '⬅️ Zurück', cmd: 'TG:NAV:BACK' }]] });
  await fakeAdapter.router.setInputMode('script_await', JSON.stringify({ scriptId: 'calib', cmd: 'CalibHeight', menuKey: 'balcony_calib' }));
  ...
  if (!sentMessages.at(-1)?.payload?.text?.includes('28.5')) throw new Error('Proaktive Rückfrage wurde nicht korrekt verarbeitet!');

  // Ja/Nein-Tastatur bei einer Skript-Rückfrage statt Freitext
  fakeAdapter.sendTo = function (instance, command, message, callback) {
    if (instance === this.config.javascriptInstance && command === 'toScript' && message.script === 'script.js.Telegram.delconfirm') {
      if (!message.data.isReply) {
  ...
  console.log('--- Ja/Nein-Tastatur bei Rückfrage ---', sentMessages.at(-1)?.payload?.text, JSON.stringify(sentMessages.at(-1)?.payload?.reply_markup?.keyboard));
  ...
  if (sentMessages.at(-1)?.payload?.text !== '🗑 Gelöscht') throw new Error('Ja/Nein-Antwort wurde nicht korrekt verarbeitet!');
  ...
  console.log('--- Admin-Benachrichtigung bei neuem Bereich ---', adminMsg?.payload?.text, JSON.stringify(adminMsg?.payload?.reply_markup?.inline_keyboard));
  if (!adminMsg) throw new Error('Admin wurde nicht mit Inline-Keyboard benachrichtigt!');
  const approveCallback = adminMsg.payload.reply_markup.inline_keyboard[0][0].callback_data;
  if (approveCallback !== 'TG:ADMIN:APPROVEAREA:brandneuerBereich') throw new Error('Falsches callback_data für Erlauben-Button!');
  ...
  if (!ackMsg) throw new Error('answerCallbackQuery wurde nicht gesendet - Ladekreis würde ewig drehen!');
  const approvedAreas = fakeAdapter.notify.getValidAreas();
  if (!approvedAreas.brandneuerBereich) throw new Error('Bereich wurde nach "Erlauben" nicht tatsächlich freigeschaltet!');
  ...
  if (!sentMessages.at(-1)?.payload?.text?.includes('Nur Admins')) throw new Error('Nicht-Admin konnte trotzdem freischalten!');
  if (fakeAdapter.notify.getValidAreas().zweiterNeuerBereich) throw new Error('Bereich wurde trotz fehlender Admin-Rechte freigeschaltet!');

  // Bild aus Datenpunkt (Base64, ein winziges 1x1-PNG)
  const tinyPngBase64 = 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=';
  await fakeAdapter.setForeignStateAsync('0_userdata.0.TestBild', { val: `data:image/png;base64,${tinyPngBase64}`, ack: true });
  await setMenu(fakeAdapter, 'main', {
    title: '🏠 Hauptmenü',
  ...
  if (writtenBuffer.length === 0) throw new Error('Temporäre Bilddatei ist leer!');
  if (imgMsg?.payload?.caption !== 'Ein Testbild') throw new Error('Bildunterschrift wurde nicht korrekt übernommen!');
  ...
  if (sentMessages.length !== 0) throw new Error('"info" hätte per Standard automatisch gebündelt werden müssen!');

  sentMessages.length = 0;
  await fakeAdapter.notify('grouptest', 'error', 'Fehler A');
  console.log('--- Standard: "error" bleibt sofort einzeln? ---', sentMessages.length);
  if (sentMessages.length !== 1) throw new Error('"error" hätte per Standard NICHT gebündelt werden dürfen!');

  // Eigene Ausschlussliste: "info" jetzt auch ausschließen -> bleibt einzeln
  await setMenu(fakeAdapter, 'main', { title: '🏠 Hauptmenü', groupableExcludeTypes: ['warn', 'error', 'info'], rows: [[{ text: '⬅️ Zurück', cmd: 'TG:NAV:BACK' }]] });
  sentMessages.length = 0;
  ...
  if (sentMessages.length !== 1) throw new Error('Explizit ausgeschlossenes "info" hätte sofort ankommen müssen!');
  ...
  if (sentMessages.length !== 0) throw new Error('Testaufbau falsch - Nachricht hätte gerade erst gepuffert werden müssen!');
  ...
  if (sentMessages.length !== 1) throw new Error('Gepufferte Nachricht wurde beim Shutdown-Flush NICHT ausgeliefert - würde bei echtem Neustart verloren gehen!');
  ...
  await setMenu(fakeAdapter, 'main', { title: '🏠 Hauptmenü', groupableExcludeTypes: ['warn', 'error', 'open'], rows: [[{ text: '⬅️ Zurück', cmd: 'TG:NAV:BACK' }]] });
  ...
  if (sentMessages.length !== 0) throw new Error('Pausierter Bereich hat trotzdem "open" zugestellt!');
  ...
  if (sentMessages.length !== 1) throw new Error('"warn" hätte trotz Pause ankommen müssen!');
  ...
  if (sentMessages.length !== 1) throw new Error('Nach Aufhebung hätte "open" wieder ankommen müssen!');
  ...
  if (nonFinalRows.some((row) => row.length > 2)) throw new Error('Benutzer-Liste hat eine Zeile mit mehr als 2 Buttons!');
  ...
  if (!nonFinalRows3.some((row) => row.length === 3)) throw new Error('Globale Einstellung "Buttons pro Zeile"=3 wurde bei Auto-Menüs nicht angewendet!');
  ...
  await handleAdminCommandForCombo(fakeAdapter, 'TG:ADMIN:USER:123456', '123456', '123456', fakeAdapter.router.renderMenu);
  ...
  await fakeAdapter.setForeignStateAsync('0_userdata.0.ToggleBild', { val: `data:image/png;base64,${tinyPngBase64}`, ack: true });
  await fakeAdapter.setForeignStateAsync('0_userdata.0.LichtToggle', { val: false, ack: true });
  await setMenu(fakeAdapter, 'main', {
    title: '🏠 Hauptmenü',
    rows: [[{ text: '💡 Licht', cmd: '', datapoint: '0_userdata.0.LichtToggle', imageDatapoint: '0_userdata.0.ToggleBild' }]],
  });
  await fakeAdapter.setStateAsync('runtime.currentMenu', { val: 'main', ack: true });
  sentMessages.length = 0;
  await fakeAdapter.router.handleIncoming('[123456] 💡 Licht');
  ...
  console.log('--- Bild + Toggle kombiniert: Toggle geschaltet? Bild dabei? ---', lightState?.val, !!toggleImgMsg?.payload?.text?.endsWith('.jpg'), toggleImgMsg?.payload?.caption);
  if (lightState?.val !== true) throw new Error('Toggle wurde beim Bild-Fall NICHT ausgeführt - genau der gemeldete Bug!');
  if (!toggleImgMsg?.payload?.text?.endsWith('.jpg')) throw new Error('Bild wurde nicht mitgeschickt!');
  ...
  fakeAdapter.sendTo = function (instance, command, message, callback) {
    if (instance === this.config.javascriptInstance && command === 'toScript' && message.script === 'script.js.Telegram.vacuum') {
  ...
  await setMenu(fakeAdapter, 'vacuum_maint', {
    ...
    rows: [
      ...
      [{ text: '⬅️ Zurück', cmd: 'TG:NAV:BACK' }, { text: '🏠 Hauptmenü', cmd: 'TG:NAV:MAIN' }],
    ],
  });
  sentMessages.length = 0;
  await fakeAdapter.router.renderMenu('123456', 'vacuum_maint');
  const maintMsg = sentMessages.at(-1);
  console.log('--- Menü-Skript: Text vom Skript, Tastatur vom Menü ---', maintMsg?.payload?.text, JSON.stringify(maintMsg?.payload?.reply_markup?.keyboard));
  if (!maintMsg?.payload?.text?.includes('Filter: 1h 44m')) throw new Error('Skript-Text wurde nicht als Menü-Antworttext verwendet!');
  if (!maintMsg?.payload?.reply_markup?.keyboard?.flat().includes('🧼 Filter zurücksetzen')) throw new Error('Menü-eigene Tastatur (Reset-Buttons) fehlt!');
  if (!maintMsg?.payload?.reply_markup?.keyboard?.flat().includes('🏠 Hauptmenü')) throw new Error('Kombi-Button fehlt in der Menü-eigenen Tastatur!');
  ...
  if (currentMenuAfterGhost?.val !== 'main') throw new Error('currentMenu wurde fälschlich auf das leere Geister-Menü gesetzt - genau der gemeldete Bug!');

  // Nächster echter Knopfdruck muss wieder normal funktionieren
  sentMessages.length = 0;
  await fakeAdapter.router.handleIncoming('[123456] ⛽️ Benzin');
  console.log('--- Knopfdruck nach Geister-Menü funktioniert wieder? ---', sentMessages.length > 0 ? 'ja' : 'NEIN, immer noch blockiert');
  if (sentMessages.length === 0) throw new Error('Navigation blieb nach dem Geister-Menü blockiert!');
  ...
  await setMenu(fakeAdapter, 'main', { title: '🏠 Hauptmenü', message: 'Status: {{0_userdata.0.TestBool}}', rows: [[{ text: '⬅️ Zurück', cmd: 'TG:NAV:BACK' }]] });
  sentMessages.length = 0;
  await fakeAdapter.router.renderMenu('123456', 'main');
  console.log('--- Boolean-Platzhalter (true) ---', sentMessages.at(-1)?.payload?.text);
  if (sentMessages.at(-1)?.payload?.text !== 'Status: ✅ an') throw new Error('Boolean "true" wurde nicht automatisch übersetzt!');
  ...
  if (sentMessages.at(-1)?.payload?.text !== 'Status: ⛔ aus') throw new Error('Boolean "false" wurde nicht automatisch übersetzt!');

  // Eigener Text für an/aus
  await setMenu(fakeAdapter, 'main', {
    title: '🏠 Hauptmenü', boolTrueText: 'JA', boolFalseText: 'NEIN',
    message: 'Status: {{0_userdata.0.TestBool}}', rows: [[{ text: '⬅️ Zurück', cmd: 'TG:NAV:BACK' }]],
  });
  sentMessages.length = 0;
  await fakeAdapter.router.renderMenu('123456', 'main');
  console.log('--- Eigener Bool-Text ---', sentMessages.at(-1)?.payload?.text);
  if (sentMessages.at(-1)?.payload?.text !== 'Status: NEIN') throw new Error('Eigener Bool-Text wurde nicht verwendet!');

  // Bool-Umwandlung komplett ausschalten -> roh true/false
  await setMenu(fakeAdapter, 'main', {
    title: '🏠 Hauptmenü', boolTranslateEnabled: false,
    message: 'Status: {{0_userdata.0.TestBool}}', rows: [[{ text: '⬅️ Zurück', cmd: 'TG:NAV:BACK' }]],
  });
  sentMessages.length = 0;
  await fakeAdapter.router.renderMenu('123456', 'main');
  console.log('--- Bool-Umwandlung ausgeschaltet ---', sentMessages.at(-1)?.payload?.text);
  if (sentMessages.at(-1)?.payload?.text !== 'Status: false') throw new Error('Bool-Umwandlung hätte ausgeschaltet sein müssen!');
  ...
  await setMenu(fakeAdapter, 'main', {
    title: '🏠 Hauptmenü', dateFormat: 'DD.MM.YYYY',
    message: 'Datum: {{0_userdata.0.TestDatum}}', rows: [[{ text: '⬅️ Zurück', cmd: 'TG:NAV:BACK' }]],
  });
  sentMessages.length = 0;
  await fakeAdapter.router.renderMenu('123456', 'main');
  console.log('--- Datums-Umwandlung ---', sentMessages.at(-1)?.payload?.text);
  if (sentMessages.at(-1)?.payload?.text !== 'Datum: 19.07.2026') throw new Error('Datum wurde nicht korrekt umgewandelt!');
  ...
  await setMenu(fakeAdapter, 'main', {
    title: '🏠 Hauptmenü', dateFormat: 'DD.MM.YYYY', dateTimeFormat: 'DD.MM.YYYY um HH:mm',
    message: 'Zeit: {{0_userdata.0.TestDatumZeit}}', rows: [[{ text: '⬅️ Zurück', cmd: 'TG:NAV:BACK' }]],
  });
  sentMessages.length = 0;
  await fakeAdapter.router.renderMenu('123456', 'main');
  console.log('--- Datum MIT Uhrzeit (eigenes Format) ---', sentMessages.at(-1)?.payload?.text);
  if (sentMessages.at(-1)?.payload?.text !== 'Zeit: 19.07.2026 um 14:30') throw new Error('Datum+Uhrzeit wurde nicht korrekt umgewandelt!');

  // Reines Datum bleibt exakt 00:00, keine Zeitzonen-Verschiebung
  await setMenu(fakeAdapter, 'main', {
    title: '🏠 Hauptmenü', dateFormat: 'DD.MM.YYYY HH:mm',
    message: 'Datum: {{0_userdata.0.TestDatum}}', rows: [[{ text: '⬅️ Zurück', cmd: 'TG:NAV:BACK' }]],
  });
  sentMessages.length = 0;
  await fakeAdapter.router.renderMenu('123456', 'main');
  console.log('--- Reines Datum mit HH:mm im Format (muss exakt 00:00 sein) ---', sentMessages.at(-1)?.payload?.text);
  if (sentMessages.at(-1)?.payload?.text !== 'Datum: 19.07.2026 00:00') throw new Error('Reines Datum zeigt nicht exakt 00:00 - Zeitzonen-Drift!');

  // Datums-Umwandlung ausgeschaltet -> roh
  await setMenu(fakeAdapter, 'main', {
    title: '🏠 Hauptmenü', dateTranslateEnabled: false,
    message: 'Datum: {{0_userdata.0.TestDatum}}', rows: [[{ text: '⬅️ Zurück', cmd: 'TG:NAV:BACK' }]],
  });
  sentMessages.length = 0;
  await fakeAdapter.router.renderMenu('123456', 'main');
  console.log('--- Datums-Umwandlung ausgeschaltet ---', sentMessages.at(-1)?.payload?.text);
  if (sentMessages.at(-1)?.payload?.text !== 'Datum: 2026-07-19') throw new Error('Datums-Umwandlung hätte ausgeschaltet sein müssen!');

  // Hauptschalter komplett aus -> auch "info" bleibt sofort einzeln, egal was in der Ausschlussliste steht
  await setMenu(fakeAdapter, 'main', { title: '🏠 Hauptmenü', groupingEnabled: false, rows: [[{ text: '⬅️ Zurück', cmd: 'TG:NAV:BACK' }]] });
  sentMessages.length = 0;
  ...
  if (sentMessages.length !== 1) throw new Error('Mit ausgeschaltetem Hauptschalter hätte "info" trotzdem sofort ankommen müssen!');
  ...
  if (!permTexts.some((t) => t.includes('Wäsche'))) throw new Error('Anzeigename aus Benachrichtigungen wurde in der Berechtigungs-Liste nicht übernommen!');

  // Vergleichsoperator als Status-Regel-Wert (z. B. "<50")
  await setMenu(fakeAdapter, 'main', {
    title: '🏠 Hauptmenü',
    rows: [[{
      text: 'Akku', cmd: 'TG:BATTERY:SHOW',
      icon: { datapoint: '0_userdata.0.Akku', rules: [{ value: '<50', emoji: '🪫', label: 'Akku schwach' }, { value: '>=50', emoji: '🔋', label: 'Akku ok' }] },
    }]],
  });
  await fakeAdapter.setStateAsync('runtime.currentMenu', { val: 'main', ack: true });
  ...
  if (!keyboard?.flat().includes('🪫 Akku schwach')) throw new Error('"<50" hat bei Wert 30 nicht gematcht!');
  ...
  if (!keyboard?.flat().includes('🔋 Akku ok')) throw new Error('">=50" hat bei Wert 80 nicht gematcht!');

  // Menü mit Bild-Datenpunkt: Bild + Antworttext (als Bildunterschrift) + Tastatur in EINER Nachricht
  await fakeAdapter.setForeignStateAsync('0_userdata.0.KameraBild', { val: `data:image/png;base64,${tinyPngBase64}`, ack: true });
  ...
  console.log('--- Menü mit Bild: caption + Tastatur ---', menuImgMsg?.payload?.caption, JSON.stringify(menuImgMsg?.payload?.reply_markup?.keyboard));
  if (menuImgMsg?.payload?.caption !== 'Letztes Bild von der Kamera') throw new Error('Menü-Antworttext wurde nicht als Bildunterschrift verwendet!');
  if (!menuImgMsg?.payload?.reply_markup?.keyboard?.flat().includes('⬅️ Zurück')) throw new Error('Tastatur fehlt bei Bild+Menü-Nachricht!');
  if (!menuImgMsg?.payload?.text?.endsWith('.jpg') || !fs.existsSync(menuImgMsg.payload.text)) throw new Error('Bild wurde nicht korrekt als Datei geschrieben!');
  ...
  await fakeAdapter.setForeignStateAsync('0_userdata.0.VacuumImage', { val: `data:image/png;base64,${tinyPngBase64}`, ack: true });
  await setMenu(fakeAdapter, 'main', {
    title: '🏠 Hauptmenü',
    rows: [[{ text: '📊 Vacuum Status', cmd: 'VacuumStatus', scriptId: 'vacuum', imageDatapoint: '0_userdata.0.VacuumImage' }]],
  });
  await fakeAdapter.setStateAsync('runtime.currentMenu', { val: 'main', ack: true });
  sentMessages.length = 0;
  ...
  if (!vacuumImgMsg?.payload?.caption?.includes('Akku: 87 %')) throw new Error('Skript-Antwort wurde nicht als Bildunterschrift verwendet!');
  if (!vacuumImgMsg?.payload?.text?.endsWith('.jpg')) throw new Error('Bild wurde nicht mitgeschickt!');
  ...
  sentMessages.length = 0;
  await fakeAdapter.notify('private', 'post.p1', '✉️ Ein Brief ist unterwegs.', { file: '/home/pi/iobroker/brief.png', caption: '✉️ Ein Brief ist unterwegs.' });
  const fileMsg = sentMessages.at(-1);
  console.log('--- notify() mit Datei-Anhang ---', fileMsg?.payload?.text, '|', fileMsg?.payload?.caption);
  if (fileMsg?.payload?.text !== '/home/pi/iobroker/brief.png') throw new Error('Dateipfad landete nicht im "text"-Feld - falsches Schema!');
  if (fileMsg?.payload?.caption !== '✉️ Ein Brief ist unterwegs.') throw new Error('Bildunterschrift falsch!');
  ...
  if (plainMsg?.payload?.parse_mode) throw new Error('parse_mode wurde ohne extra.html trotzdem gesetzt!');
  ...
  console.log('--- notify() mit extra.html=true: parse_mode ---', htmlMsg?.payload?.parse_mode, '|', htmlMsg?.payload?.text);
  if (htmlMsg?.payload?.parse_mode !== 'HTML') throw new Error('parse_mode:"HTML" wurde mit extra.html=true nicht gesetzt!');
  if (htmlMsg?.payload?.text !== '<b>Fett</b> und normal') throw new Error('HTML-Text wurde unerwartet verändert!');
  ...
  if (htmlShorthandMsg?.payload?.parse_mode !== 'HTML') throw new Error('Kurzform extra:"html" hat parse_mode nicht gesetzt!');
  ...
  console.log('--- notify() mit extra="nopreview": disable_web_page_preview ---', noPreviewMsg?.payload?.disable_web_page_preview);
  if (noPreviewMsg?.payload?.disable_web_page_preview !== true) throw new Error('Kurzform extra:"nopreview" hat disable_web_page_preview nicht gesetzt!');

  // html + noPreview kombiniert via Kurzform "html,nopreview"
  sentMessages.length = 0;
  await fakeAdapter.notify('private', 'error', '<a href="https://maps.apple.com/?q=Test">Route</a>', 'html,nopreview');
  const combinedMsg = sentMessages.at(-1);
  console.log('--- notify() mit extra="html,nopreview" ---', combinedMsg?.payload?.parse_mode, combinedMsg?.payload?.disable_web_page_preview);
  ...
  const simulated = simulateNotifyMessage({ area: 'private', type: 'salary.p1', text: 'Entgeltnachweis da', file: '/x.pdf' });
  console.log('--- Ohne eigenes caption-Feld: text wird automatisch Bildunterschrift ---', simulated.fileExtra);
  if (simulated.fileExtra.caption !== 'Entgeltnachweis da') throw new Error('text wurde nicht automatisch zur Bildunterschrift!');
  ...
  if (sentMessages.at(-1)?.payload?.text !== 'Laufzeit: 2 Minuten') throw new Error('Rechenoperation im Platzhalter falsch!');
  ...
  if (sentMessages.at(-1)?.payload?.text !== '3 min') throw new Error('Verkettete Rechenoperation falsch!');
  ...
  const adminUserMsg = sentMessages.find((m) => m.payload?.user === '123456' && m.payload?.reply_markup?.inline_keyboard);
  console.log('--- Admin-Benachrichtigung bei neuem Nutzer ---', adminUserMsg?.payload?.text, JSON.stringify(adminUserMsg?.payload?.reply_markup?.inline_keyboard));
  if (!adminUserMsg) throw new Error('Admin wurde nicht über wartenden Nutzer benachrichtigt!');
  const approveUserCallback = adminUserMsg.payload.reply_markup.inline_keyboard[0][0].callback_data;
  if (approveUserCallback !== 'TG:ADMIN:APPROVEUSER:555555') throw new Error('Falsches callback_data für Nutzer-Erlauben-Button!');
  ...
  if (denyCallback !== 'TG:ADMIN:DENYUSER:666666') throw new Error('Falsches callback_data für Nutzer-Ablehnen-Button!');
  sentMessages.length = 0;
  ...
  if (deniedUserObj) throw new Error('Abgelehnter Nutzer wurde nicht gelöscht!');
  ...
  // Exakter Treffer
  await handleEventTriggerStateChange(fakeAdapter, '0_userdata.0.Test.Alarm', { val: true, ack: true }, mockRenderMenu);
  ...
  await handleEventTriggerStateChange(fakeAdapter, '0_userdata.0.Test.Alarm', { val: false, ack: true }, mockRenderMenu);
  if (triggeredCalls.length) throw new Error('Event-Trigger hat trotz falschem Wert ausgelöst!');
  ...
  if (!triggeredCalls.some((c) => c.menuKey === 'trigger_test_menu_gt')) throw new Error('Event-Trigger mit Operator ">25" hat bei 27 nicht ausgelöst!');

  triggeredCalls.length = 0;
  await handleEventTriggerStateChange(fakeAdapter, '0_userdata.0.Test.Temp', { val: 20, ack: true }, mockRenderMenu);
  if (triggeredCalls.length) throw new Error('Event-Trigger mit Operator ">25" hat bei 20 fälschlich ausgelöst!');

  // Nicht abonnierter Datenpunkt -> no-op, kein Fehler
  await handleEventTriggerStateChange(fakeAdapter, '0_userdata.0.Irrelevant.Datenpunkt', { val: true, ack: true }, mockRenderMenu);

  // Nicht freigeschalteter Nutzer bekommt das Menü nicht
  const { key: unapprovedKey } = await ensureUser(fakeAdapter, '777778');
  triggeredCalls.length = 0;
  await handleEventTriggerStateChange(fakeAdapter, '0_userdata.0.Test.Alarm', { val: true, ack: true }, mockRenderMenu);
  if (triggeredCalls.some((c) => c.chatId === '777778')) throw new Error('Event-Trigger hat einen nicht freigeschalteten Nutzer benachrichtigt!');
  if (!triggeredCalls.some((c) => c.chatId === '777777')) throw new Error('Event-Trigger hat den freigeschalteten Nutzer bei erneutem Treffer nicht mehr benachrichtigt!');
  ...
  await setMenu(fakeAdapter, 'http_test_menu', {
    title: 'HTTP Test',
    rows: [[
      { text: '🌐 Webhook auslösen', httpUrl: 'https://example.invalid/webhook', httpMethod: 'POST', httpBody: '{"x":1}' },
      { text: '🔐 Mit Auth', httpUrl: 'https://example.invalid/secure', httpAuthUser: 'bob', httpAuthPass: 'secret', message: 'Status: {status} | {response}' },
    ]],
  });
  ...
  if (fetchCalls.at(-1)?.url !== 'https://example.invalid/webhook') throw new Error('HTTP-Button hat die falsche URL aufgerufen!');
  if (fetchCalls.at(-1)?.opts?.method !== 'POST') throw new Error('HTTP-Button hat nicht die konfigurierte Methode (POST) verwendet!');
  if (fetchCalls.at(-1)?.opts?.body !== '{"x":1}') throw new Error('HTTP-Button hat den Body nicht mitgeschickt!');
  const httpConfirmMsg = sentMessages.at(-1);
  console.log('--- HTTP-Button Bestätigung ---', httpConfirmMsg?.payload?.text);
  if (!httpConfirmMsg?.payload?.text?.includes('HTTP 200')) throw new Error('HTTP-Button-Bestätigung zeigt nicht den Status-Code!');
  ...
  if (authHeader !== expectedAuth) throw new Error('HTTP-Button hat den Basic-Auth-Header nicht korrekt gesetzt!');
  ...
  if (!httpErrorMsg?.payload?.text?.includes('fehlgeschlagen')) throw new Error('HTTP-Button hat den Fehlerfall nicht sauber abgefangen!');
