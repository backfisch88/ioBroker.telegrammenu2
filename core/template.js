...
function classifyDateLike(val) {
  ...
  if (DATE_TIME_RE.test(s)) return 'datetime';
  return null;
}
...
async function getGlobalPlaceholderSettings(adapter) {
  try {
    ...
    return { boolEnabled: true, trueText: '✅ an', falseText: '⛔ aus', dateEnabled: true, dateFormat: 'DD.MM.YYYY', dateTimeFormat: 'DD.MM.YYYY HH:mm' };
  }
}
...
function formatPlaceholderValue(val, settings) {
  if (settings.boolEnabled) {
    if (val === true || val === 'true') return settings.trueText;
    if (val === false || val === 'false') return settings.falseText;
  }
  if (settings.dateEnabled) {
    ...
    if (kind === 'datetime') {
      ...
      if (!isNaN(d.getTime())) return formatDateWithPattern(d, settings.dateTimeFormat);
    }
  }
  return String(val);
}
...
async function resolveIcon(adapter, iconConfig, fallbackEmoji) {
  if (!iconConfig || !iconConfig.datapoint) return fallbackEmoji || '';
...
async function resolveIconRule(adapter, iconConfig) {
  if (!iconConfig || !iconConfig.datapoint) return null;
  try {
    const state = await adapter.getForeignStateAsync(iconConfig.datapoint);
    const val = state ? String(state.val) : '';
