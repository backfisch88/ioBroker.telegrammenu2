...
const { ensureUser, userToKey, hasPermission, isAdmin, isApproved, approveUser, denyUser, notifyAdminsAboutPendingUser } = require('./users');
...
function createRouter(adapter) {
  ...
  async function getHistory() {
    ...
    try { return JSON.parse(s?.val || '[]'); } catch (e) { return []; }
  }
  ...
  async function clearInputMode() { await setInputMode('', ''); }
  ...
  async function handleDatapointInput(user, userKey, rawText) {
    ...
    let ctx;
    try { ctx = JSON.parse(contextState?.val || '{}'); } catch (e) { ctx = {}; }
    ...
    try {
      ...
      if (ctx.menuKey) await renderMenu(user, ctx.menuKey, confirmText);
      else await sendText(user, confirmText);
    } catch (e) {
  ...
  async function handleNumpadInput(user, userKey, key) {
    ...
    if (key === '❌ Abbrechen') {
      await clearInputMode();
      if (ctx.menuKey) await renderMenu(user, ctx.menuKey, '❌ Abgebrochen.');
      else await sendText(user, '❌ Abgebrochen.');
      return;
    }
    ...
    if (key === '✅ Fertig') {
      ...
      if (!ctx.buffer || Number.isNaN(num)) {
        ...
        await sendMenu(user, numpadPromptText(`⚠️ Ungültige Zahl, bitte neu eingeben. ${ctx.prompt || ''}`, ''), NUMPAD_ROWS);
        return;
      }
      if (ctx.min !== undefined && ctx.min !== '' && num < Number(ctx.min)) {
        await setInputMode('datapoint_numpad', JSON.stringify({ ...ctx, buffer: '' }));
        await sendMenu(user, numpadPromptText(`⚠️ Mindestens ${ctx.min}, bitte neu eingeben. ${ctx.prompt || ''}`, ''), NUMPAD_ROWS);
        return;
      }
      if (ctx.max !== undefined && ctx.max !== '' && num > Number(ctx.max)) {
        await setInputMode('datapoint_numpad', JSON.stringify({ ...ctx, buffer: '' }));
        await sendMenu(user, numpadPromptText(`⚠️ Höchstens ${ctx.max}, bitte neu eingeben. ${ctx.prompt || ''}`, ''), NUMPAD_ROWS);
        return;
      }
      await clearInputMode();
      try {
    ...
    if (key === '⌫') buffer = buffer.slice(0, -1);
    else if (key === '.') { if (!buffer.includes('.')) buffer += '.'; }
    else if (/^[0-9]$/.test(key)) buffer += key;
    else return; // unbekannte Eingabe (z. B. versehentlich Text) -> ignorieren, kein State-Update nötig
  ...
  async function handleScriptReply(user, userKey, text) {
    ...
    const result = await adapter.scriptBridge.callScript(ctx.scriptId, ctx.cmd, { value: text, user, isReply: true });
    ...
    if (result.awaitReply) {
      await setInputMode('script_await', JSON.stringify({ scriptId: ctx.scriptId, cmd: ctx.cmd, menuKey: ctx.menuKey }));
      if (result.keyboard) await sendMenu(user, result.text, result.keyboard, opts);
      else await sendTextNoKeyboard(user, result.text, opts);
      return;
    }

    if (result.menuKey) await renderMenu(user, result.menuKey, result.text, opts);
    else if (ctx.menuKey) await renderMenu(user, ctx.menuKey, result.text, opts);
    else await sendText(user, result.text, opts);
  }

  async function buildButtonText(btn) {
    if (!btn.icon) return resolveTemplate(adapter, btn.text);
    ...
    if (String(btn.text || '').trim().startsWith('{{')) {
  ...
  async function buildKeyboard(rows, userKey) {
    ...
    for (const row of rows || []) {
      ...
      for (const btn of row) {
        ...
        if (btn.perm && !(await hasPermission(adapter, btn.perm, userKey))) continue;
  ...
  function normalizeSendOpts(opts) {
    if (typeof opts === 'boolean') return { html: opts, noPreview: false };
  ...
  async function sendInlineMenu(user, text, inlineRows, opts = false) {
    ...
    const payload = {
      text,
      user,
      reply_markup: { inline_keyboard: inlineRows.map((row) => row.map((btn) => ({ text: btn.text, callback_data: btn.cmd }))) },
    };
    if (html) payload.parse_mode = 'HTML';
    if (noPreview) payload.disable_web_page_preview = true;
    await adapter.sendToAsync(adapter.telegramInstance, payload);
  ...
  async function startNumpadInput(user, { datapoint, min, max, menuKey, prompt, message }) {
    ...
    await setInputMode('datapoint_numpad', JSON.stringify({
      datapoint,
      min,
      max,
  ...
  async function renderMenu(user, menuKey, customText, customOpts = false) {
    ...
    if (rows.length > 0) {
      ...
      adapter.log.warn(`renderMenu: "${menuKey}" hat keine Buttons - currentMenu bleibt unverändert, um Navigation nicht zu blockieren.`);
    }
    ...
    if (title === undefined) {
      if (menuDef.scriptId && menuDef.cmd) {
        ...
        if (result && result.text) {
          ...
          adapter.log.warn(`Skript ${menuDef.scriptId} hat auf "${menuDef.cmd}" nicht geantwortet - Menü-Titel fällt auf Antworttext/Titel zurück.`);
        }
      }
      if (title === undefined && menuDef.message) {
        ...
        if (menuDef.icon) {
          ...
          if (emoji) title = title.replace(/^\S+/, emoji);
        }
      }
    }
  ...
  async function findButtonByText(menuKey, text, userKey) {
    const menuDef = await getMenu(adapter, menuKey);
    ...
    if (menuDef.source) {
      ...
      try { rows = JSON.parse(cached?.val || '[]'); } catch (e) { rows = []; }
    }

    for (const row of rows) {
      for (const btn of row) {
        if (btn.text === text) return btn;
        if (btn.icon && (await buildButtonText(btn)) === text) return btn;
        if (btn.text && btn.text.includes('{{') && (await resolveTemplate(adapter, btn.text)) === text) return btn;
      }
    }
  ...
  async function dispatchCommand(cmd, value, user, userKey) {
    ...
    if (await handleAdminCommand(adapter, cmd, user, userKey, renderMenu, sendText)) return;
    if (await handleNotifyToggle(adapter, cmd, user, userKey, renderMenu)) return;
    if (await handleValueSetCommand(adapter, cmd, user, userKey, renderMenu)) return;
    if (await handleValueCustomCommand(adapter, cmd, user, userKey, startNumpadInput)) return;
  ...
  async function handleIncoming(raw) {
    ...
    if (text.startsWith('TG:ADMIN:APPROVEAREA:') || text.startsWith('TG:ADMIN:DENYAREA:')) {
      ...
      if (isApprove) {
        ...
        await adapter.sendToAsync(adapter.telegramInstance, { user, answerCallbackQuery: { text: `"${area}" freigeschaltet`, showAlert: false } });
        await sendText(user, `✅ Bereich "${area}" freigeschaltet.`);
      } else {
        await adapter.notify.deleteArea(area);
        await adapter.sendToAsync(adapter.telegramInstance, { user, answerCallbackQuery: { text: `"${area}" abgelehnt`, showAlert: false } });
    ...
    if (text.startsWith('TG:ADMIN:APPROVEUSER:') || text.startsWith('TG:ADMIN:DENYUSER:')) {
      ...
      if (isApprove) {
        ...
        await adapter.sendToAsync(adapter.telegramInstance, { user, answerCallbackQuery: { text: `"${targetKey}" freigeschaltet`, showAlert: false } });
        ...
        await adapter.sendToAsync(adapter.telegramInstance, { user, answerCallbackQuery: { text: `"${targetKey}" abgelehnt`, showAlert: false } });
    ...
    if (button.imageDatapoint) {
      try {
        ...
        if (button.scriptId && button.cmd) {
          const result = await adapter.scriptBridge.callScript(button.scriptId, button.cmd, { value: button.text, user });
          if (result && result.text) {
            caption = result.text;
            captionHtml = !!result.html;
            captionNoPreview = !!(result.noPreview || result.disablePreview);
          }
          else adapter.log.warn(`Skript ${button.scriptId} hat auf "${button.cmd}" nicht geantwortet - Bild wird ohne Skript-Text gesendet.`);
          ...
          const offVal = button.offValue !== undefined && button.offValue !== '' ? coerceValue(button.offValue) : false;
          try {
            const current = await adapter.getForeignStateAsync(button.datapoint);
            const isOn = current ? current.val === onVal || current.val === true || String(current.val) === String(onVal) : false;
            const nextVal = isOn ? offVal : onVal;
            await adapter.setForeignStateAsync(button.datapoint, { val: nextVal, ack: false });
            if (!caption) caption = `${isOn ? '❌' : '✅'} ${button.text || ''} → ${nextVal}`.trim();
          } catch (e) {
            adapter.log.warn(`Datenpunkt-Toggle für ${button.datapoint} (mit Bild) fehlgeschlagen: ${e.message}`);
          }
        }
        const imgPayload = { text: filePath, caption, user };
        if (captionHtml) imgPayload.parse_mode = 'HTML';
        if (captionNoPreview) imgPayload.disable_web_page_preview = true;
    ...
    if (button.icon && button.icon.datapoint) {
      ...
      if (rule && rule.writeDatapoint) {
        try {
          const writeVal = coerceValue(rule.writeValue !== undefined && rule.writeValue !== '' ? rule.writeValue : 'true');
          ...
          let confirmText;
          if (button.message) {
            const withStatus = button.message.replace(/\{status\}/g, rule.statusText || rule.label || String(writeVal));
            confirmText = await resolveTemplate(adapter, withStatus);
          } else {
            confirmText = `✅ ${rule.label || button.text || ''}${rule.statusText ? ` ${rule.statusText}` : ` → ${writeVal}`}`.trim();
          }
    ...
    if (button.httpUrl) {
      ...
      if (button.httpBody && method !== 'GET') headers['Content-Type'] = 'application/json';
    ...
    if (button.datapoint) {
      ...
      if (button.inputType === 'text') {
        ...
        const currentMenuKey = (await adapter.getStateAsync('runtime.currentMenu'))?.val || 'main';
        await setInputMode('datapoint_input', JSON.stringify({
          datapoint: button.datapoint,
    ...
    if (button.scriptId && button.cmd) {
      ...
      if (result && result.text) {
        ...
        if (result.awaitReply) {
          ...
          const currentMenuKey = (await adapter.getStateAsync('runtime.currentMenu'))?.val || 'main';
          await setInputMode('script_await', JSON.stringify({ scriptId: button.scriptId, cmd: button.cmd, menuKey: currentMenuKey }));
          if (result.keyboard) await sendMenu(user, result.text, result.keyboard, opts);
          else await sendTextNoKeyboard(user, result.text, opts);
  ...
  return { handleIncoming, renderMenu, dispatchCommand, setInputMode, sendTextNoKeyboard, sendMenu, startNumpadInput };
}
