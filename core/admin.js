...
async function collectKnownPerms(adapter) {
  ...
  for (const key of keys) {
    ...
    if (!def) continue;
    if (def.perm && def.perm !== 'admin') perms.add(def.perm);
    for (const row of def.rows || []) {
      for (const btn of row) {
        if (btn.perm && btn.perm !== 'admin') perms.add(btn.perm);
      }
    }
  }
...
async function buildUserDetailMenu(adapter, targetUserKey) {
  ...
  const rows = [
    [{ text: `${role === 'admin' ? '👑' : '👤'} Rolle: ${role} (antippen zum Wechseln)`, cmd: `TG:ADMIN:USER:${targetUserKey}:ROLE:TOGGLE` }],
  ];
  ...
  rows.push([{ text: '⬅️ Zurück', cmd: 'TG:NAV:BACK' }, { text: '🏠 Hauptmenü', cmd: 'TG:NAV:MAIN' }]);
...
async function handleAdminCommand(adapter, cmd, requestUser, requestUserKey, renderMenuFn, sendTextFn) {
  if (!cmd || !cmd.startsWith('TG:ADMIN:USER:')) return false;
