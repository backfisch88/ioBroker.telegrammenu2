...
function createNotifyEngine(adapter) {
  ...
  function ensureAreaEntry(area, approvedDefault) {
    if (!validAreas[area]) {
      validAreas[area] = { types: new Set(), approved: approvedDefault, override: new Set(), labels: {}, areaLabel: '', pausedUntil: 0 };
    }
  ...
  async function persist() {
    ...
    for (const [area, entry] of Object.entries(validAreas)) {
      obj[area] = { types: [...entry.types], approved: !!entry.approved, override: [...entry.override], labels: entry.labels || {}, areaLabel: entry.areaLabel || '', pausedUntil: entry.pausedUntil || 0 };
    }
    ...
    try {
      ...
      for (const [area, entry] of Object.entries(obj)) {
        ...
        if (Array.isArray(entry)) {
          validAreas[area] = { types: new Set(entry), approved: true, override: new Set(), labels: {}, areaLabel: '', pausedUntil: 0 };
        } else {
          validAreas[area] = {
  ...
  async function setOverride(area, type, isOverride) {
    const entry = ensureAreaEntry(area, true);
    if (isOverride) entry.override.add(type);
    else entry.override.delete(type);
    await persist();
  }

  async function setTypeLabel(area, type, label) {
    const entry = ensureAreaEntry(area, true);
    if (label) entry.labels[type] = label;
    else delete entry.labels[type];
    await persist();
  }
  ...
  async function deleteArea(area) {
    if (!validAreas[area]) return false;
  ...
  async function isGloballyGroupableType(type) {
    try {
      ...
      const excludeTypes = Array.isArray(mainDef?.groupableExcludeTypes) ? mainDef.groupableExcludeTypes : ['warn', 'error'];
  ...
  async function send(area, type, text, extra = null) {
    ...
    if (changed) {
      if (!entry.approved) adapter.log.info(`notify: neuer Bereich "${area}" automatisch entdeckt, wartet auf Freischaltung (approveArea)`);
      await persist();
    }
    ...
    if (isAreaPaused(area) && type !== 'warn' && type !== 'error') {
      adapter.log.debug(`notify: Bereich "${area}" pausiert bis ${new Date(entry.pausedUntil).toISOString()} - Nachricht verworfen`);
      return;
    }

    const users = await adapter.getStateAsync('runtime.usersJson');
    let userKeys = [];
    try { userKeys = JSON.parse(users?.val || '[]'); } catch (e) { userKeys = []; }
    ...
    for (const userKey of userKeys) {
      ...
      if (!isOverride) {
        ...
        const notifyEnabled = notifyPrefState ? (notifyPrefState.val === true || notifyPrefState.val === 'true') : true;
      ...
      if (extra && extra.file) {
        ...
        if (html) filePayload.parse_mode = 'HTML';
        if (noPreview) filePayload.disable_web_page_preview = true;
  ...
  async function flushAllPending() {
    ...
    for (const userKey of userKeys) {
      const chatState = await adapter.getStateAsync(`users.${userKey}.chatId`);
      const chatId = chatState?.val;
      if (chatId) await flush(userKey, chatId);
    }
    clearTimers();
  }

  return { init, send, registerAreas, getValidAreas, getPendingAreas, approveArea, setOverride, isOverride, setTypeLabel, getLabel, setAreaLabel, getAreaLabel, deleteArea, getAllAreasFull, clearTimers, flushAllPending, pauseArea, unpauseArea, isAreaPaused, getAreaPausedUntil };
}
