...
const BACK_MAIN_ROW = [{ text: '⬅️ Zurück', cmd: 'TG:NAV:BACK' }, { text: '🏠 Hauptmenü', cmd: 'TG:NAV:MAIN' }];
...
async function buildValueRangeRows(adapter, datapoint, min, max, step, unit, rowLength, menuKey) {
  if (!datapoint) return [[{ text: '⚠️ Kein Datenpunkt konfiguriert', cmd: 'TG:NAV:BACK' }]];

  let current = null;
  try {
    const state = await adapter.getForeignStateAsync(datapoint);
    if (state && state.val !== null && state.val !== undefined) current = Number(state.val);
  } catch (e) {
    current = null;
  }
  ...
  const values = [];
  if (ascending) {
    for (let v = min; v <= max + 1e-9; v = round2(v + s)) values.push(round2(v));
  } else {
    for (let v = min; v >= max - 1e-9; v = round2(v - s)) values.push(round2(v));
  }
  ...
  const rows = [];
  for (let i = 0; i < buttons.length; i += rowLength) rows.push(buttons.slice(i, i + rowLength));
  if (!rows.length) rows.push([{ text: '⚠️ Kein Wertebereich konfiguriert', cmd: 'TG:NAV:BACK' }]);
...
async function buildUserRows(adapter, buttonsPerRow) {
  ...
  if (!rows.length) rows.push([{ text: 'Noch keine Nutzer registriert', cmd: 'TG:NAV:BACK' }]);
...
async function collectVisibleAreaTypes(adapter, userKey) {
  ...
  for (const [area, types] of Object.entries(areas)) {
    ...
    for (const type of typeList) {
      ...
      if (adapter.notify.isOverride && adapter.notify.isOverride(area, type)) continue;
      ...
      if (!groups.has(displayText)) groups.set(displayText, []);
...
async function buildNotifyPrefRows(adapter, userKey, buttonsPerRow) {
  ...
  const rows = chunkButtons(buttons, buttonsPerRow);
  if (!rows.length) rows.push([{ text: 'Keine Benachrichtigungsbereiche verfügbar', cmd: 'TG:NAV:BACK' }]);
...
async function buildNotifyPauseRows(adapter, userKey, buttonsPerRow) {
  ...
  const rows = chunkButtons(buttons, buttonsPerRow);
  if (!rows.length) rows.push([{ text: 'Keine Bereiche verfügbar', cmd: 'TG:NAV:BACK' }]);
  rows.push(BACK_MAIN_ROW);
  return rows;
}
