...
class TelegramMenu2 extends utils.Adapter {
  ...
  async onReady() {
    ...
    if (!existingMenus.length) {
      ...
      this.log.info('Standard-Registry importiert (Hauptmenü + Einstellungen) – kein manueller Import nötig für den ersten Start.');
    }
  ...
  onStateChange(id, state) {
    if (id.startsWith(`${this.namespace}.registry.`)) {
      setupEventTriggers(this).catch((e) => this.log.warn(`setupEventTriggers (Rescan nach Registry-Änderung): ${e.message}`));
      return;
    }
    if (!state || state.val === undefined || state.val === null) return;
    ...
    handleEventTriggerStateChange(this, id, state, (chatId, menuKey) => this.router.renderMenu(chatId, menuKey)).catch((e) =>
      this.log.warn(`Event-Listener-Verarbeitung für ${id}: ${e.message}`),
    );
  }
  ...
  detectTypeFromText(text) {
    ...
    if (/läuft/i.test(text)) return { type: 'start', text };
    if (/update/i.test(text)) return { type: 'update', text };
    if (/fertig/i.test(text)) return { type: 'done', text };
  ...
  onMessage(obj) {
    ...
    if (obj.command === 'registerScriptCommand') {
      ...
      (async () => {
        if (!cmdPrefix || !scriptId) {
          if (obj.callback) this.sendTo(obj.from, obj.command, { ok: false, error: 'cmdPrefix und scriptId erforderlich' }, obj.callback);
          return;
        }
    ...
    if (obj.command === 'awaitReply') {
      ...
      (async () => {
        if (!user || !scriptId || !cmd) {
          if (obj.callback) this.sendTo(obj.from, obj.command, { ok: false, error: 'user, scriptId und cmd erforderlich' }, obj.callback);
          return;
        }
        await this.router.setInputMode('script_await', JSON.stringify({ scriptId, cmd, menuKey: menuKey || '' }));
        if (text && keyboard) await this.router.sendMenu(user, text, keyboard);
        else if (text) await this.router.sendTextNoKeyboard(user, text);
        if (obj.callback) this.sendTo(obj.from, obj.command, { ok: true }, obj.callback);
      })();
      return;
    }
    ...
    if (obj.command === 'getScriptCommands') {
      if (obj.callback) this.sendTo(obj.from, obj.command, { ok: true, handlers: this.scriptBridge.getAll() }, obj.callback);
      return;
    }
    ...
    if (!obj.command || obj.command === 'send') {
      ...
      const fromAdapter = fromParts[0] === 'system' && fromParts[1] === 'adapter' && fromParts[2]
        ? fromParts[2]
        : (fromParts[0] || 'external');

      if (typeof obj.message === 'string' && obj.message) {
        const { type, text } = this.detectTypeFromText(obj.message);
        this.log.info(`Kompatibilitäts-Nachricht von ${fromAdapter} (String, Typ "${type}") - über notify() geroutet`);
        this.notify(fromAdapter, type, text);
        if (obj.callback) this.sendTo(obj.from, obj.command || 'send', { ok: true }, obj.callback);
        return;
      }
      ...
      if (obj.message && typeof obj.message === 'object' && obj.message.text) {
        ...
        if (target) {
          this.log.info(`Kompatibilitäts-Nachricht von ${fromAdapter} mit explizitem Ziel (${target}) - direkt weitergeleitet`);
          this.sendToAsync(this.telegramInstance, { text: obj.message.text, user: target })
            .then(() => { if (obj.callback) this.sendTo(obj.from, obj.command || 'send', { ok: true }, obj.callback); })
            .catch((e) => {
              this.log.warn(`Weiterleitung an ${this.telegramInstance} fehlgeschlagen: ${e.message}`);
              if (obj.callback) this.sendTo(obj.from, obj.command || 'send', { ok: false, error: e.message }, obj.callback);
            });
        } else {
          const { type, text } = this.detectTypeFromText(obj.message.text);
          this.log.info(`Kompatibilitäts-Nachricht von ${fromAdapter} (Objekt, kein Ziel, Typ "${type}") - über notify() geroutet`);
          this.notify(fromAdapter, type, text);
          if (obj.callback) this.sendTo(obj.from, obj.command || 'send', { ok: true }, obj.callback);
        }
        return;
    ...
    if (obj.command === 'getLayout') {
      (async () => {
        ...
        let layout = {};
        try { layout = JSON.parse(state?.val || '{}'); } catch (e) { layout = {}; }
        if (obj.callback) this.sendTo(obj.from, obj.command, { ok: true, layout }, obj.callback);
      })();
      return;
    }
    ...
    if (obj.command === 'deleteRegistryMenus') {
      ...
      (async () => {
        ...
        if (removed.length) this.log.info(`Menüs entfernt (nicht mehr im Editor vorhanden): ${removed.join(', ')}`);
        if (obj.callback) this.sendTo(obj.from, obj.command, { ok: true, removed }, obj.callback);
      })();
      return;
    }
    ...
    if (obj.command === 'getRegistry') {
      (async () => {
        try {
          ...
          if (obj.callback) this.sendTo(obj.from, obj.command, { ok: true, registry: out }, obj.callback);
        } catch (e) {
    ...
    if (obj.command === 'getNotifyAreas') {
      ...
      if (obj.callback) this.sendTo(obj.from, obj.command, { ok: true, areas }, obj.callback);
      return;
    }
    ...
    if (obj.command === 'registerNotifyAreas') {
      ...
      (async () => {
        ...
        if (obj.callback) this.sendTo(obj.from, obj.command, { ok: true, areas: Object.keys(areasMap) }, obj.callback);
      })().catch((e) => {
    ...
    if (obj.command === 'importRegistry') {
      importRegistry(this, obj.message)
        .then((keys) => {
          if (obj.callback) this.sendTo(obj.from, obj.command, { ok: true, imported: keys }, obj.callback);
        })
        .catch((e) => {
    ...
    this.log.info(`Unbekannter/nicht behandelter Message-Command: "${obj.command}" - Inhalt: ${JSON.stringify(obj.message)}`);
    if (obj.callback) this.sendTo(obj.from, obj.command, { ok: false, error: `Command "${obj.command}" wird noch nicht unterstützt` }, obj.callback);
  }
  ...
  async onUnload(callback) {
    try {
      ...
      if (this._notifyEngine) await this._notifyEngine.flushAllPending();
      callback();
    } catch (e) {
      callback();
    }
